...
{[}4{]}, {[}5{]}. Thus by surveying the collection of rare variants that
an individual carries provides information about the population origins
of that individual's recent ancestors. One of us (J.N.) recently took
advantage of the geographic clustering of an individual's rare variants
in helping to reaffirm the historically highly contested Native American
ancestry of the Kennewick Man, an ancient individual from Washington
State \textbf{???}.
...
\caption[Schematic diagram of the Geography of Genetic Variants (GGV) web application]{Schematic diagram of the Geography of Genetic Variants (GGV) web application. We compute minor allele frequencies from publicly available population genetic datasets from the 1000 Genomes Project, Human Genom Diversity Project (HGDP) and Population Reference Sample (POPRES). These allele freqeuncies are stored in a SQL database along side with geographic coordinates associated with each population. The front-end visualization was created using d3 and accesses allele frequency and population meta-data from a REST API implemented in the python library flask.}\label{fig:figure1}
\end{figure}
\end{CodeChunk}
...
\caption[Example map from the Geography of Genetic Variants web application]{Example map from the Geography of Genetic Variants web application. Each pie chart represents a population with the blue slice of the pie displaying the freqeuncy of the global minor allele and the yellow slice of the pie displaying the frequency of the global major allele in each population.}\label{fig:figure2}
\end{figure}
\end{CodeChunk}
...
\subsection*{Query API by rsid}\label{query-api-by-rsid}
\addcontentsline{toc}{subsection}{Query API by rsid}
...
\subsection*{Query API by chromosome
position}\label{query-api-by-chromosome-position}
\addcontentsline{toc}{subsection}{Query API by chromosome position}
...
\subsection*{Randomly query API}\label{randomly-query-api}
\addcontentsline{toc}{subsection}{Randomly query API}
