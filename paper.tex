...
light on where the variant first arose, in what populations it spread
...
distribution of genetic variants. Through an efficient SQL database,
REST API and dynamic front-end the
\href{popgen.uchicago.edu/ggv}{\textit{Geography of Genetic Variants (GGV)}}
browser rapidly provides maps of allele frequencies in populations
...
has identified one or many genetic variants of interest (e.g.~from a
...
population?); 2) design of follow-up studies (e.g.~what population
should be studied to observe variant carriers?); or 3) the biology of
the variant in question (e.g.~does the variant correlate with a known
environmental factor in a manner suggestive of some geographically
localized selection pressure?) {[}1{]}, {[}2{]}, {[}3{]}. A simple
geographic map of the distribution of a genetic variant can be
incredibly insightful for these questions.

The geographic distribution of rare genetic variants (minor allele
frequency \(\leq .05\)) can be particularly informative in determining
an individual's ancestry. Rare variants tend to be localized in small
geographic regions because they have arisen recently in human history
and have had little time to spread across populations through migration
events {[}4{]}, {[}5{]}. Surveying the collection of rare variants that
an individual carries can provide insight into the geographic origins of
that individual's recent ancestors. One of us (J.N.) recently took
advantage of the geographic localization of an individual's rare
variants in helping to reaffirm the historically contested Native
American ancestry of the Kennewick Man, an ancient individual from
Washington State \textbf{???}.
...
millions of single nucleotide variants (SNVs). One common approach to
...
principal components analysis {[}6{]}, {[}7{]}, or a model-based
clustering method such as STRUCTURE {[}8{]} or ADMIXTURE {[}9{]}. While
these approaches are extremely valuable, researchers can use them to
often without inspecting the underlying variant data in more detail. A
natural approach to gaining more insight to the overall structure of a
...
of data wrangling methods {[}10{]} and geographic plotting techniques
...
Our work is inspired by past tools such as the ALFRED database {[}11{]}
and the maps available on the HGDP Selection browser {[}12{]}. One of us
(J.N.) developed the scripts for the HGDP Selection Browser maps using
The Generic Mapping Tools (GMT) {[}13{]}, a powerful system of
geographic plotting scripts with a substantial learning curve. The plots
from the HGDP Selection Browser have proved useful, and have appeared in
research articles e.g {[}12{]}, {[}14{]}, books e.g. {[}15{]}, and have
been ported and made available on the UCSC Genome Browser (available
under the HGDP Allele Freq track of the browser).

Reference datasets for population genetic variation has greatly expanded
since the release of the HGDP Illumina 650Y dataset {[}16{]} that formed
the basis of the HGDP Selection Browser maps. The most notable advance
is the publication of the 1000 Genomes Phase 3 data {[}17{]} though
additional datasets are coming online {[}18{]}, {[}19{]}.
...
development community, and ease of sharing {[}20{]}.
...
\caption[Schematic diagram of the Geography of Genetic Variants (GGV) browser]{Schematic diagram of the Geography of Genetic Variants (GGV) browser. We compute minor allele frequencies from publicly available population genetic datasets from the 1000 Genomes Project, Human Genome Diversity Project (HGDP) and Population Reference Sample (POPRES). These allele frequencies are stored in a SQL database along side with geographic coordinates associated with each population. The front-end visualization was created using d3 and accesses allele frequency and population meta-data from a REST API implemented in the python library flask.}\label{fig:figure1}
\end{figure}
\end{CodeChunk}
...
project and POPRES project, we have developed an SQL database and REST
API for querying allele frequencies by chromosome and position or
reference SNP identifier {[}Figure 1{]}.

Users can query single variants by chromosome and position identifiers,
by rsid {[}21{]}, or users can simply choose a random variant from
...
\caption[Example map from the Geography of Genetic Variants browser]{Example map from the Geography of Genetic Variants browser. Each pie chart represents a population with the blue slice of the pie displaying the frequency of the global minor allele and the yellow slice of the pie displaying the frequency of the global major allele in each population.}\label{fig:figure2}
\end{figure}
\end{CodeChunk}
...
\caption[Example map from the Geography of Genetic Variants browser displaying the use of varying transparency of population pie charts to represent uncertainty in allele frequencies]{Example map from the Geography of Genetic Variants browser displaying the use of varying transparency of population pie charts to represent uncertainty in allele frequencies. The transparency is scaled in proportion to the number of observed chromosomes in each population for a particular variant.}\label{fig:figure3}
\end{figure}
\end{CodeChunk}
...
to contain rare variants {[}17{]}, {[}4{]}, {[}22{]}.
...
\caption[Example maps from the Geography of Genetic Variants browser displaying the use of frequency scales for more expressive representations of rare variation on geographic maps]{Example maps from the Geography of Genetic Variants browser displaying the use of frequency scales for more expressive representations of rare variation on geographic maps. The blue pie charts convey a give minor allele frequency out of 100 percent, the green out 1 percent, and the red out of .01 percent.}\label{fig:figure4}
\end{figure}
\end{CodeChunk}
...
\caption[Example maps from the Geography of Genetic Variants browser displaying the use of a force directed layout to limit visual clutter when many populations overlap in geographic position]{Example maps from the Geography of Genetic Variants browser displaying the use of a force directed layout to limit visual clutter when many populations overlap in geographic position. The left map shows the original population locations while the right shows the application of the force directed layout.}\label{fig:figure5}
\end{figure}
\end{CodeChunk}
...
To interface population minor allele frequency data stored in the SQL
database with the front-end D3.js visualization we use a REST API
implemented in the python library Flask-RESTful \textbf{???}. GET
requests are submitted to the database via HTTP, returning json
formatted allele frequency data and the meta-data associated with each
population and genetic variant e.g.~latitude, longitude, population
label, sample size, and frequency scale. Genetic variants can be queried
by chromosome position, rsid, or randomly. Example HTTP requests and
json response can be seen below:

\subsection*{Query by rsid}\label{query-by-rsid}
\addcontentsline{toc}{subsection}{Query by rsid}
...
\subsection*{Query by chromosome
position}\label{query-by-chromosome-position}
\addcontentsline{toc}{subsection}{Query by chromosome position}
...
\subsection*{Random query}\label{random-query}
\addcontentsline{toc}{subsection}{Random query}
...
has inherent complexity. For example, practitioners must make choices
...
CA198933-0). The authors would also like to thank members of the
Novembre Lab for supportive conversations.
...
\hypertarget{refs}{}
\hypertarget{ref-rosenberg2010genome}{}
1. Rosenberg NA, Huang L, Jewett EM, Szpiech ZA, Jankovic I, Boehnke M.
Genome-wide association studies in diverse populations. Nature Reviews
Genetics. Nature Publishing Group; 2010;11: 356--366.

\hypertarget{ref-novembre2009spatial}{}
2. Novembre J, Di Rienzo A. Spatial patterns of variation due to natural
selection in humans. Nature Reviews Genetics. Nature Publishing Group;
2009;10: 745--755.

\hypertarget{ref-coop2010using}{}
3. Coop G, Witonsky D, Di Rienzo A, Pritchard JK. Using environmental
correlations to identify loci underlying local adaptation. Genetics.
Genetics Soc America; 2010;185: 1411--1423.

\hypertarget{ref-nelson2012abundance}{}
4. Nelson MR, Wegmann D, Ehm MG, Kessner D, Jean PS, Verzilli C, et al.
...
\hypertarget{ref-mathieson2014demography}{}
5. Mathieson I, McVean G. Demography and the age of rare variants. PLoS
Genet. Public Library of Science; 2014;10: e1004528.

\hypertarget{ref-price2006principal}{}
6. Price AL, Patterson NJ, Plenge RM, Weinblatt ME, Shadick NA, Reich D.
...
7. Patterson N, Price AL, Reich D. Population structure and
eigenanalysis. PLoS genet. Public Library of Science; 2006;2: e190.

\hypertarget{ref-pritchard2000inference}{}
8. Pritchard JK, Stephens M, Donnelly P. Inference of population
...
10. Furche T, Gottlob G, Libkin L, Orsi G, Paton NW. Data wrangling for
big data: Challenges and opportunities.

\hypertarget{ref-rajeevan2011alfred}{}
11. Rajeevan H, Soundararajan U, Kidd JR, Pakstis AJ, Kidd KK. ALFRED:
An allele frequency resource for research and teaching. Nucleic acids
research. Oxford Univ Press; 2011; gkr924.

\hypertarget{ref-pickrell2009signals}{}
12. Pickrell JK, Coop G, Novembre J, Kudaravalli S, Li JZ, Absher D, et
...
13. Wessel P, Smith WH, Scharroo R, Luis J, Wobbe F. Generic mapping
tools: Improved version released. Eos, Transactions American Geophysical
Union. Wiley Online Library; 2013;94: 409--410.

\hypertarget{ref-coop2009role}{}
14. Coop G, Pickrell JK, Novembre J, Kudaravalli S, Li J, Absher D, et
al. The role of geography in human adaptation. PLoS Genet. Public
Library of Science; 2009;5: e1000500.

\hypertarget{ref-dudley2013exploring}{}
15. Dudley JT, Karczewski KJ. Exploring personal genomics. Oxford
University Press; 2013.

\hypertarget{ref-li2008worldwide}{}
16. Li JZ, Absher DM, Tang H, Southwick AM, Casto AM, Ramachandran S, et
...
20. Bostock M, Ogievetsky V, Heer J. D\(^3\) data-driven documents.
Visualization and Computer Graphics, IEEE Transactions on. IEEE;
2011;17: 2301--2309.

\hypertarget{ref-sherry2001dbsnp}{}
21. Sherry ST, Ward M-H, Kholodov M, Baker J, Phan L, Smigielski EM, et
...
22. Tennessen JA, Bigham AW, O'Connor TD, Fu W, Kenny EE, Gravel S, et
