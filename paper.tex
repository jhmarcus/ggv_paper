...
distribution of genetic variants. Through an efficient REST API and
dynamic front-end the
...
events {[}4{]}, {[}5{]}. Thus by surveying the collection of rare
variants that an individual carries provides information about the
population origins of that individual's recent ancestors. One of us
(J.N.) recently took advantage of the geographic clustering of an
individual's rare variants in helping to reaffirm the historically
highly contested Native American ancestry of the Kennewick Man, an
ancient individual from Washington State \textbf{???}.
...
principal components analysis {[}6{]}, {[}7{]}, a model-based clustering
method such as STRUCTURE {[}8{]} or ADMIXTURE {[}9{]}. While these
...
geographic plotting scripts with a sbustantial learning curve. The plots
from the HGDP Selection Browser have proved useful, and have appeared in
research articles e.g {[}12{]}, {[}14{]}, books e.g. {[}15{]}, and have
been ported and made avialable on the UCSC Genome Browser (available
...
development community, and ease of share-ability {[}20{]}.
...
\caption[Schematic diagram of the Geography of Genetic Variants (GGV) web application]{Schematic diagram of the Geography of Genetic Variants (GGV) web application. We compute minor allele frequencies from publicly available population genetic datasets from the 1000 Genomes Project, Human Genome Diversity Project (HGDP) and Population Reference Sample (POPRES). These allele frequencies are stored in a SQL database along side with geographic coordinates associated with each population. The front-end visualization was created using d3 and accesses allele frequency and population meta-data from a REST API implemented in the python library flask.}\label{fig:figure1}
\end{figure}
\end{CodeChunk}
...
\caption[Example map from the Geography of Genetic Variants web application displaying the use of varying transparency of population pie charts to represent uncertainty in allele frequencies]{Example map from the Geography of Genetic Variants web application displaying the use of varying transparency of population pie charts to represent uncertainty in allele frequencies. The transparency is scaled in proportion to the number of observed chromosomes for a particular variant in each population.}\label{fig:figure3}
\end{figure}
\end{CodeChunk}
...
\caption[Example map from the Geography of Genetic Variants web application displaying the use of a force directed layout to limit visual clutter when many populations overlap in geographic position]{Example map from the Geography of Genetic Variants web application displaying the use of a force directed layout to limit visual clutter when many populations overlap in geographic position. The left map shows the original population locations while the right shows the application of the force directed.}\label{fig:figure5}
\end{figure}
\end{CodeChunk}
...
impelemented in the python library Flask-RESTful \textbf{???}. GET
