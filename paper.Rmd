...
```{r figure3, echo=FALSE, message=FALSE, fig.height=6, fig.width=6, fig.align='center', fig.cap='Example map from the Geography of Genetic Variants web application displaying the use of varying transparency of population pie charts to represent uncertainty in allele frequencies. The transparency is scaled in proportion to the number of observed chromosomes for a particular variant in each population.'}
...
```{r figure5, echo=FALSE, message=FALSE, fig.height=6, fig.width=6, fig.align='center', fig.cap='Example map from the Geography of Genetic Variants web application displaying the use of a force directed layout to limit visual clutter when many populations overlap in geographic position. The left map shows the original population locations while the right shows the application of the force directed.'}
img <- readPNG("figures/png/figure5.png")
grid.raster(img)
```

# Underlying Frequency API {-}
To interface population minor allele frequency data stored in the SQL database with the front-end D3.js visualization we use a REST API impelemented in the python library Flask-RESTful @citeXXX. GET requests are submitted to the database via HTTP, returning json formatted allele frequency data and the meta-data associated with each population and genetic variant e.g. latitude, longitude, population label, sample size, and frequency scale. Genetic variants can be queried by chromosome position, rsid, or randomly. Example HTTP requests and json response can be seen below:
